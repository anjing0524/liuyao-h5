/* @ts-self-types="./liuyao_core.d.ts" */
 ...
 * @param {number} cast_at_ms
 * @param {number | null} [rng_seed]
 * @returns {any}
 */
export function cast_and_install(question, cast_at_ms, rng_seed) {
    const ptr0 = passStringToWasm0(question, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.cast_and_install(ptr0, len0, cast_at_ms, isLikeNone(rng_seed) ? Number.MAX_SAFE_INTEGER : (rng_seed) >>> 0);
    return ret;
}
 ...
 * @param {Array<any>} lines_yang
 * @param {Array<any>} line_kinds
 * @param {number} cast_at_ms
 * @returns {any}
 */
export function install_only(lines_yang, line_kinds, cast_at_ms) {
    const ret = wasm.install_only(lines_yang, line_kinds, cast_at_ms);
    return ret;
}
 ...
 * @returns {any}
 */
export function roll_one_line() {
    const ret = wasm.roll_one_line();
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('liuyao_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
